...
import { calculateSavingsProgress, formatCurrency, type SavingsStyle } from "@/lib/savings-goal";
...
interface SavingsGoalPreviewProps {
  title?: string;
  current?: number;
  target?: number;
  currency?: string;
...
export default function SavingsGoalPreview({
  title = "",
  current = 0,
  target = 1000000,
  currency = "₩",
  ...
  fontSize = "md",
  font = "sans",
}: SavingsGoalPreviewProps) {
  const { percentage } = calculateSavingsProgress(current, target);
