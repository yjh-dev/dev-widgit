...
import { useRatingCardStore } from "@/store/useRatingCardStore";
...
export default function CreateRatingCardPage() {
  const {
    title, rating, maxStars, label,
    color, textColor, font, bg, transparentBg,
    borderRadius, padding, fontSize,
    setTitle, setRating, setMaxStars, setLabel,
  ...
  const { buildWidgetUrl, widgetUrl } = useWidgetUrl(() => {
    const base = `${window.location.origin}/widget/rating-card`;
    const params = new URLSearchParams();
    if (title) params.set("title", title);
    if (rating !== 4.5) params.set("rating", String(rating));
    if (maxStars !== 5) params.set("maxStars", String(maxStars));
  ...
  }, [title, rating, maxStars, label, color, textColor, font, bg, transparentBg, borderRadius, padding, fontSize, fx, fxInt, gbg, gbgDir, neonColor, bshadow, tshadow, bw, bc, opacity, ls]);
  ...
  return (
    <EditorLayout title="별점 카드 위젯 만들기">
      <Card>
        <CardContent className="pt-6">
          <EditorSection
            ...
            sections={[
                ...
                children: (
                  <>
                    <div className="space-y-2">
                      ...
                      <Input id="title" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="영화 / 책 / 제품 이름" />
                    </div>
                    <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input id="label" value={label} onChange={(e) => setLabel(e.target.value)} placeholder="예: 5.0 만점" />
                    </div>
                  </>
                ),
              },
              ...
              },
              {
                ...
                children: (
                  <>
                    <ColorPicker id="color" label="별 색상" value={color} onChange={setColor} placeholder="F59E0B" />
                    <ColorPicker id="textColor" label="텍스트 색상 (비우면 별 색상)" value={textColor} onChange={setTextColor} placeholder="" />
          ...
          />
          <div className="mt-6">
            <EditorActions widgetUrl={widgetUrl} onCopy={handleCopy} onReset={reset} onApplyTheme={(c) => useRatingCardStore.setState(c)} />
          </div>
        </CardContent>
      </Card>
      ...
      <div className="flex items-center justify-center order-first md:order-last md:sticky md:top-8">
        <div className="space-y-3 w-full max-w-[400px]">
          ...
          <div className="border rounded-lg overflow-hidden aspect-[16/9]">
            ...
            >
              <RatingCardPreview
                title={title} rating={rating} maxStars={maxStars} label={label}
