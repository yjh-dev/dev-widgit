...
export default function CreateSavingsGoalPage() {
  const {
    title, current, target, currency, style, showValue,
    color, textColor, font, bg, transparentBg,
    borderRadius, padding, fontSize,
    setTitle, setCurrent, setTarget, setCurrency, setStyle, setShowValue,
  ...
  useInitFromUrl((p) => {
    loadPreset({
      ...
      ...(p.has("currency") && { currency: p.get("currency")! }),
      ...(p.has("style") && { style: p.get("style") as SavingsStyle }),
  ...
  const { buildWidgetUrl, widgetUrl } = useWidgetUrl(() => {
    const base = `${window.location.origin}/widget/savings-goal`;
    const params = new URLSearchParams();
    if (title) params.set("title", title);
    if (current !== 0) params.set("current", String(current));
    if (target !== 1000000) params.set("target", String(target));
  ...
  }, [title, current, target, currency, style, showValue, color, textColor, font, bg, transparentBg, borderRadius, padding, fontSize, fx, fxInt, gbg, gbgDir, neonColor, bshadow, tshadow, bw, bc, opacity, ls]);
  ...
  return (
    <EditorLayout title="저축 목표 위젯 만들기">
      <Card>
        <CardContent className="pt-6">
          <EditorSection
            ...
            sections={[
                ...
                children: (
                  <>
                    <div className="space-y-2">
                      ...
                      <Input id="title" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="저축 목표 이름" />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="target">목표 금액</Label>
                      <Input
                ...
                children: (
                  <>
                    <div className="space-y-2">
                      ...
                      <Select value={style} onValueChange={(v) => setStyle(v as SavingsStyle)}>
                        <SelectTrigger className="w-full"><SelectValue /></SelectTrigger>
                        <SelectContent>
                    ...
                    <div className="flex items-center justify-between">
                      <Label htmlFor="showValue">금액 표시</Label>
              ...
              },
              {
                ...
                children: (
                  <>
                    <ColorPicker id="color" label="프로그레스 색상" value={color} onChange={setColor} placeholder="22C55E" />
          ...
          />
          <div className="mt-6">
            <EditorActions widgetUrl={widgetUrl} onCopy={handleCopy} onReset={reset} onApplyTheme={(c) => useSavingsGoalStore.setState(c)} />
          </div>
        </CardContent>
      </Card>
      ...
      <div className="flex items-center justify-center order-first md:order-last md:sticky md:top-8">
        <div className="space-y-3 w-full max-w-[400px]">
          ...
          <div className="border rounded-lg overflow-hidden aspect-[16/9]">
            ...
            >
              <SavingsGoalPreview
                title={title} current={current} target={target} currency={currency}
