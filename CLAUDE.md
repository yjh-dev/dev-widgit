...
- **콘텐츠 & 장식** (21): 명언 카드, 텍스트 배너, 음악 플레이어, 날씨, 달 위상, 메모지, 그라데이션, 타이프라이터, QR 코드, 주사위, 통계 카드, 컬러 팔레트, 구분선, 이미지 카드, 환율, 레이더 차트, 도넛 차트, 이모지 비, 변경 로그, 포춘 쿠키, 뱃지
